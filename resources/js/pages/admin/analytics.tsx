        ...
        <AdminLayout>
            ...
            <div className="space-y-6 p-6">
                <header className="flex flex-wrap items-end justify-between gap-4">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            Data live maksimum {retentionDays} hari; periode
                            lebih lama tersedia di tabel arsip.
                        </p>
                    </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    <MetricCard
                        title={
                            tracking.eventLabels[EVENT_TYPES.visit] ??
                            EVENT_TYPES.visit
                        }
                    ...
                    <MetricCard
                        title={
                            tracking.eventLabels[EVENT_TYPES.engagement] ??
                    ...
                    />
                    {tracking.capabilities.total_lead && (
                        <MetricCard
                            title="Total Lead"
                            value={stats.total_leads.toLocaleString()}
                            hint={`${stats.lead_cr}% Lead CR`}
                        />
                    )}
                    {tracking.mode === 'form' && (
                        <MetricCard
                            title={
                                tracking.eventLabels[EVENT_TYPES.lead] ??
                                EVENT_TYPES.lead
                            }
                            value={stats.total_leads.toLocaleString()}
                            hint={`${stats.lead_cr}% Lead CR`}
                    ...
                    {tracking.mode === 'form' && (
                        <MetricCard
                            title={
                                tracking.eventLabels[EVENT_TYPES.payment] ??
                                EVENT_TYPES.payment
                            }
                            value={stats.payments.toLocaleString()}
                            hint={`${stats.sales_cr}% Sales CR`}
                        />
                    )}
                    ...
                    {tracking.capabilities.revenue && (
                        <MetricCard title="RPV" value={money(stats.rpv)} />
                    )}
                </div>
