...
import { getMaximumScrollDepth, sendHeartbeat, track } from '@/analytics/tracker';
...
export function useEngagement(config: TrackingProps): void {
    useEffect(() => {
        ...
        const tick = () => {
            ...
            activeSeconds += 1;
            if (activeSeconds >= config.engagementThreshold && !sessionStorage.getItem(engagementKey)) {
                sessionStorage.setItem(engagementKey, '1');
                void track(EVENT_TYPES.engagement, { duration_seconds: activeSeconds });
            }
        };

        const heartbeat = () => sendHeartbeat(activeSeconds, getMaximumScrollDepth());
        const activeTimer = window.setInterval(tick, 1_000);
        const heartbeatTimer = window.setInterval(heartbeat, config.heartbeatInterval * 1_000);
