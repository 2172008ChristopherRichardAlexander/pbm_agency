...
export type LeadResponse = { lead_id: number; redirect_url: string; event_id: string };
...
export function TrackedForm({ formName, endpoint = '/lead', onSuccess, onError, children, ...props }: Props) {
    ...
    const submit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        const metaEventId = window.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(36).slice(2)}`;
        const fields = Object.fromEntries(form.entries());
        const params = new URLSearchParams(window.location.search);
        const standard = ['name', 'email', 'phone'];
        const extra = Object.fromEntries(Object.entries(fields).filter(([key]) => !standard.includes(key)));
        const payload = {
            name: fields.name,
            email: fields.email,
            phone: fields.phone,
        ...
        try {
            ...
            trackServerConfirmed(EVENT_TYPES.lead, lead.event_id, { landing_source: landingSource() });
            ...
            if (tracking.paymentMode === 'internal') {
                ...
                if (!checkout.ok) throw new Error(order.message ?? 'Checkout failed.');
